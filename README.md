...
# Use your own container or conda env (see Quick Start above)
# Place pretrained weights under ./pretrained_models/
# Place CrossDocked (or custom) data under ./data/
```
...
```bash
# Pocket quality evaluation (after generation)
python evaluate_pocket_quality.py \
    --pt_file outputs/result_0_*.pt \
    --protein_root ./data/crossdocked_v1.1_rmsd1.0_pocket10

# Optional scatter plot from evaluation Excel
...
Pretrained weights (`pretrained_models/`), datasets (`data/`), and runtime outputs (`outputs/`, `diffdynamic.db`) are **not** shipped in this repository. Local `server/server_config.json` is gitignored — generate it on first run.
...
├── scripts/
│   └── sample_diffusion.py      # Main sampling script
├── utils/
│   └── evaluation/
...
├── server/                      # FastAPI backend + job scheduler
├── ui/                          # Web frontend
