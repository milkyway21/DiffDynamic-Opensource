...
│   ├── config.py            # 运行时配置（首次启动可生成本地 server_config.json）
...
│       └── css/style.css    # 样式
│
├── scripts/sample_diffusion.py              # 分子生成
...
> 预训练权重（`pretrained_models/`）、数据集（`data/`）与运行产物（`outputs/`、`diffdynamic.db`）不包含在本仓库中，需自行准备。`server/server_config.json` 为本地运行时配置，请勿提交。
...
所有采样参数在 `configs/sampling.yml` 中：
