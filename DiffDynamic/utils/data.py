...
class PDBProtein(object):  # 解析 PDB 蛋白质文件并生成结构化信息的工具类。
    ...
    def __init__(self, data, mode='auto'):
        ...
        if (mode == 'auto' and (data[-4:].lower() == '.pdb' or data[-6:].lower() == '.pdbqt')) or mode == 'path':  # 根据模式判断是路径还是原始文本。
    ...
    def _enum_formatted_atom_lines(self):  # 枚举并标准化 PDB 文件中的原子与表头行。
        for line in self.block.splitlines():  # 遍历每一行。
            if line[0:6].strip() == 'ATOM':  # 处理原子记录。
                element_symb = line[76:78].strip() if len(line) > 77 else ''
                # Validate: RDKit periodic table only accepts standard element symbols.
                # PDBQT files put atom types (HD, OA, NA, SA, A) at 76:78 which are not elements.
                # Always derive from atom name first letter for safety.
                atom_name_raw = line[12:16].strip()
                first_alpha = ''
                for ch in atom_name_raw:
                    if ch.isalpha():
                        first_alpha = ch.upper()
                        break
                # Use column 76:78 only if it's a known single-char element or matches atom name
                if element_symb and len(element_symb) == 1 and first_alpha and element_symb.upper() == first_alpha:
                    pass  # Single-char element matching atom name prefix — OK
                elif first_alpha:
                    element_symb = first_alpha  # Default to first letter of atom name
                elif len(element_symb) == 0:
                    element_symb = line[13:14].upper()  # Last resort
                yield {  # 生成标准化字典。
                    'line': line,  # 原始文本行。
                    'type': 'ATOM',  # 行类型。
