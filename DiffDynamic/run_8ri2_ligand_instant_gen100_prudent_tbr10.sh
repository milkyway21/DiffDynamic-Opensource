#!/usr/bin/env bash
# 8RI2 ligand：preserve 零分配侧链 + 100 瞬时/完整生成 + valid-Vina n100 + 新 violin
# Usage: GPU=1 bash run_8ri2_ligand_instant_gen100_prudent_tbr10.sh
set -eo pipefail
...
OUT="${OUT:-experiments/8ri2_ligand_instant_gen100_prudent_tbr10_fragprior_adaptgate_lockpos_radial_scafbond}"
CFG="${CFG:-configs/run_8ri2_ligand_instant_gen100_prudent_tbr10.yml}"
PROTEIN="/data/ye/protein-ligand/8RI2/8RI2_apo.pdb"
LIGAND="/data/ye/protein-ligand/8RI2/8RI2_ligand.sdf"
GPU="${GPU:-2}"
BASE_SEED="${BASE_SEED:-42}"

mkdir -p "$OUT"/{instant,final,run,reconstructed_molecules_n100,plots}
echo "OUT=$OUT GPU=$GPU START=$(date -Iseconds)"

# ---------- 1) 100 instant SDFs（与 sample_diffusion 对齐，含保留侧链）----------
python3 - <<PY
import json
...
sc0.SetProp('_Name', '8RI2_ligand_murcko_scaffold')
w.write(sc0); w.close()
...
for sample_idx in range(N):
    ...
    preserved_pos, _pv, preserved_indices, zero_ids = resolve_preserved_sidechains(
        sites, meta, sites_cfg, ref_pos, ref_log_v,
    )
    ...
    if sample_idx < 5 or sample_idx % 20 == 0:
        print(f'[{sample_idx:03d}] n_extra={n_placed} n_preserved={n_preserved} zero={zero_ids} seed={rng_seed}')
...
echo "========== Pack finals =========="
OUT_DIR="$OUT" python3 scripts/pack_8ri2_ligand_instant_gen100_prudent_tbr10.py

# ---------- 5) valid-Vina n100 + 新 palette plot ----------
...
python3 scripts/select_n100_valid_and_plot.py \
  ...
  --tag 8RI2_ligand_radial_scafbond_n100 \
  --prefix 8RI2_radial_scafbond_n100 \
