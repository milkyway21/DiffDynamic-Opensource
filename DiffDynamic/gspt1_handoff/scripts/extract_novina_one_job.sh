#!/usr/bin/env bash
# 单 job：evaluate_pt_with_correct_reconstruct --vina-modes none
# Usage:
#   TARGET=ikzf2 JOB_ID=0 bash extract_novina_one_job.sh
set -eo pipefail
...
OUT="$ROOT/$TARGET/extract_novina/$JOB_TAG"
...
# Require at least one reconstructed SDF under OUT (script may nest eval_* dir)
