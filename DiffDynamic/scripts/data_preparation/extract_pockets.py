...
import multiprocessing as mp  # 导入多进程模块，加速处理。
import pickle  # 导入 pickle，保存索引。
import shutil  # 导入 shutil，复制文件。
from functools import partial  # 导入 partial，用于构建偏函数。

from tqdm.auto import tqdm  # 导入 tqdm，显示进度。

from utils.data import PDBProtein, parse_sdf_file  # 导入数据解析工具。


def load_item(item, path):
    """读取指定口袋样本的蛋白 PDB 块与配体 SDF 块。"""
