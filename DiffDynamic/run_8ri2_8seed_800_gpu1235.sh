#!/usr/bin/env bash
# 8RI2：8 个随机种子 × 100 = 800 分子，GPU 1/2/3/5 各跑 2 个 seed（波次内 4 卡并行）
# Usage: bash run_8ri2_8seed_800_gpu1235.sh
set -eo pipefail
...
ROOT="experiments/8ri2_8seed_800_radial_scafbond"
...
run_one_seed() {
  ...
  local cfg="configs/run_8ri2_seed_${seed}_800.yml"
  ...
  OUT="$out" BASE_SEED="$seed" CFG="$cfg" GPU="$gpu" \
    ...
    > "outputs/8ri2_seed${seed}_gpu${gpu}.log" 2>&1
  local ec=$?
