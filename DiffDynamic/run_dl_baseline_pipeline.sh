#!/usr/bin/env bash
# dl_baseline production pipeline: generate + eval + active-ligand comparison plots
# Usage:
#   bash run_dl_baseline_pipeline.sh [JOB ...]
#   JOB: 6w63_b30 | 6w63_b50 | 8ri2_b30 | 8ri2_b50 | all (default: all)
#   VERSION=v2 (default) controls output directory suffix
#
# Runs in two waves (same target shares one GPU, cross-target parallel):
#   Wave1: 6w63_b30 + 8ri2_b30
#   Wave2: 6w63_b50 + 8ri2_b50
set -eo pipefail
...
resolve_scaffold() {
  ...
  local base
  base=$(basename "$DIR")
  if [[ "$base" == 6w63_* || "$DIR" == *6w63* ]]; then
    for f in experiments/6w63prudent_murckosites_v1/scaffold/6W63_murcko_scaffold.sdf \
             experiments/6w63prudent_murckosites_v2/scaffold/6W63_murcko_scaffold.sdf; do
      [[ -f "$f" ]] && echo "$f" && return
    done
  elif [[ "$base" == 8ri2_* || "$DIR" == *8ri2* ]]; then
    for f in experiments/8ri2prudent_murckosites_v1/scaffold/8RI2_murcko_scaffold.sdf \
             experiments/8ri2prudent_murckosites_v2/scaffold/8RI2_murcko_scaffold.sdf; do
      [[ -f "$f" ]] && echo "$f" && return
    done
  fi
}
...
run_wave() {
  local pids=()
  if want "$1"; then
    run_job "$1" "$2" "$3" "$4" "$P6W63_PROTEIN" "$P6W63_LIGAND" \
      "$P6W63_ACT_CSV" "$P6W63_ACT_SDF" "$GPU_6W63" &
    pids+=($!)
  fi
  if want "$5"; then
    run_job "$5" "$6" "$7" "$8" "$P8RI2_PROTEIN" "$P8RI2_LIGAND" \
...
fail=0
run_wave 6w63_b30 "experiments/6w63_dl_baseline30_${VERSION}" configs/run_6w63_dl_baseline30.yml "6W63_dl_baseline30_${VERSION}" \
         8ri2_b30 "experiments/8ri2_dl_baseline30_${VERSION}" configs/run_8ri2_dl_baseline30.yml "8RI2_dl_baseline30_${VERSION}" || fail=1
run_wave 6w63_b50 "experiments/6w63_dl_baseline50_${VERSION}" configs/run_6w63_dl_baseline50.yml "6W63_dl_baseline50_${VERSION}" \
         8ri2_b50 "experiments/8ri2_dl_baseline50_${VERSION}" configs/run_8ri2_dl_baseline50.yml "8RI2_dl_baseline50_${VERSION}" || fail=1
