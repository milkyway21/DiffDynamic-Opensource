#!/usr/bin/env bash
# 8RI2：scaffold dynamic_locked 生成 100 → 评估 → n100 出图
# Usage: GPU=0 bash run_8ri2_scaffold_dynamic_locked_gen100.sh
set -eo pipefail
...
OUT="${OUT:-experiments/8ri2_scaffold_dl_gen100_gpu${GPU}_${DATE_TAG}}"
CFG_SRC="${CFG:-configs/run_8ri2_scaffold_dynamic_locked_gen100.yml}"
...
python3 scripts/select_n100_valid_and_plot.py \
  ...
  --tag 8RI2_scaffold_dl_n100 \
  --prefix 8RI2_scaffold_dl_n100 \
